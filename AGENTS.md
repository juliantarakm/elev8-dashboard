...
- **Data + types**: `app/components/key-management/data/keys.ts`
  - `PhysicalKey` type with `id`, `code`, `name`, `listingId`, `status: 'available' | 'checked_out' | 'lost' | 'retired'`, `currentCheckout?: KeyCheckout`, `notes`, `createdAt`, `updatedAt`
  - `KeyCheckout` type with `guestName`, `guestEmail`, `guestPhone`, `checkedOutAt`, `expectedReturnAt`, `returnedAt?`, `checkedOutBy`, `notes`
  - `KeyActivity` type with `id`, `keyId`, `type: 'registered' | 'checked_out' | 'returned' | 'lost' | 'retired'`, `timestamp`, `staffId`, `notes`
  - `KeyStatus` union and helpers: `keyStatusLabel`, `keyStatusColor`, `formatKeyCode`
  - Mock data: 6 keys across 3 listings, mix of available/checked-out/lost statuses
- **State**: `app/composables/useKeyManagement.ts`
  - `keys` uses `useState<PhysicalKey[]>` persisted to `localStorage` key `elev8-keys`
  - `activities` uses `useState<KeyActivity[]>` persisted to `localStorage` key `elev8-key-activities`
  - Actions: `registerKey()`, `checkoutKey()`, `returnKey()`, `markLost()`, `retireKey()`, `updateNotes()`, `reactivateKey()`
  - Computed: `availableKeys`, `checkedOutKeys`, `lostKeys`, `retiredKeys`, `overdueKeys`, `keysByListing`, `getKeyById()`, `getKeyActivities()`
  - Overdue check: compares `expectedReturnAt` with current date at local midnight
- **Alerts**: `app/components/notifications/data/alerts.ts` includes `KEY_NOT_RETURNED` alert type
  - Generated by `tests/utils/key-alert.ts` utility using `useNotifications().addKeyNotReturnedAlert()`
  - Severity `WARNING`, routes to `/key-management`
- **Page**: `app/pages/key-management/index.vue`
  - Stats cards: Total keys, Available, Checked out, Lost
  - `KeyTable.vue` with status badges, listing name, guest info, due date, actions
  - Filter bar: search by key code/name, status tabs, listing popover filter, overdue toggle
  - Key box grid: `KeyBoxCard.vue` cards per listing showing available/checked-out/lost counts
  - Dialogs: `KeyRegisterDialog.vue` (register new key), `KeyCheckoutDialog.vue` (check out to guest), `KeyLostDialog.vue` (mark lost/retire), `KeyBoxDialog.vue` (listing key box detail), `KeyHistorySheet.vue` (activity timeline)
  - Activity timeline: `KeyActivityTimeline.vue` showing register/check-out/return/lost events with staff avatars
